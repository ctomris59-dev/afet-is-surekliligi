...
/* ---------------------------------------------------------------
   RENK PALETİ (afet-is-surekliligi editorial slate/red teması)
--------------------------------------------------------------- */
...
function drawHeaderBanner(doc, logoBase64) {
  ...
  doc.text("Afet & İş Sürekliliği Skorkartı Raporu", titleX, 15);
}
...
function footer(doc) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text("ÇORLU TİCARET VE SANAYİ ODASI · AFET VE İŞ SÜREKLİLİĞİ SKORKARTI", MARGIN, PAGE_H - 7);
...
/* ---------------------------------------------------------------
   ANA RAPOR ÜRETİCİ
--------------------------------------------------------------- */
export async function generateAfetPdfReport({
  companyName,
  contactName,
  dimensions,
  overall,
  ...
  const calloutLines = doc.splitTextToSize(
    "Bu rapor sonuçlarınızı detaylandırmak ve iş sürekliliği planınızı birlikte oluşturmak için Odamız uzmanlarıyla iletişime geçebilirsiniz.",
    CONTENT_W - 10
  );
  ...
  doc.save(`corlu-tso-afet-is-surekliligi-${safeName}.pdf`);
}
